...
int main() {
    ...
    for (int i=0, ai; i<n; i++) {
        cin >> ai;
        all.push_back((ll)ai);
        if (ai < 0) minus.push_back((ll)ai);
        else if (ai > 0) plus.push_back((ll)ai);
    }
    int zero = n - plus.size() - minus.size();
    if (zero + k > n) { // all zero
        cout << 0 << '\n';
        return 0;
    }
    ...
    for (int i=0 ; i<k/2; i++) {
        if (mi + 1 < minus.size() && (pi + 1 >= plus.size() || plus[pi] * plus[pi+1] < minus[mi] * minus[mi+1])) {
            ans *= minus[mi] * minus[mi+1];
            mi += 2;
        } else if (pi + 1 < plus.size() && (mi + 1 >= minus.size() || minus[mi] * minus[mi+1] <= plus[pi] * plus[pi+1])) {
    ...
    // less than or eq to 0
    ans = 1;
    sort(all.begin(), all.end(), [](ll a, ll b) {
        return abs(a) < abs(b);
    });
