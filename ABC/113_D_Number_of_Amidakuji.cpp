...
int main() {
    int h, w, k;
    cin >> h >> w >> k;
    modlong fib[] = {1, 2, 3, 5, 8, 13, 21, 34};
    modlong dp[h+1][w];
    fill(dp[0], dp[h+1], 0);
    dp[0][0] = 1;
    for (int i=0; i<h; i++) {
        for (int j=0; j<w; j++) {
            dp[i+1][j] += dp[i][j] * fib[relu(j-1)] * fib[relu(w-j-2)];
            if (j > 0)
                dp[i+1][j-1] += dp[i][j] * fib[relu(j-2)] * fib[relu(w-j-2)];
            if (j < w-1)
                dp[i+1][j+1] += dp[i][j] * fib[relu(j-1)] * fib[relu(w-j-3)];
        }
    }
    cout << dp[h][k-1] << '\n';
    return 0;
