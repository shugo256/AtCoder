...
int main() {
    ...
    for (int i=0; i<n; i++) {
        ...
        a /= g; b /= g;
        cnt[{a, b}]++;
        P key = minmax(abs(a), abs(b));
        if (key == P{a, b} || key == P{-a, -b} || key == P{b, -a} || key == P{-b, a})
            pairs[key] = {cnt[{a, b}] + cnt[{-a, -b}], cnt[{b, -a}] + cnt[{-b, a}]};
        else
            pairs[{key.fi, -key.se}] = {cnt[{a, b}] + cnt[{-a, -b}], cnt[{b, -a}] + cnt[{-b, a}]};
    }
    modlong ans = modlong(2).pow(n - zeros);
    for (auto &p:pairs) {
        ll k = p.se.fi + p.se.se;
        ans /= modlong(2).pow(k);
        ans *= modlong(2).pow(k) - (modlong(2).pow(p.se.fi) - 1) * (modlong(2).pow(p.se.se) - 1);
    } 
    cout << ans - 1 + zeros << '\n';
    return 0;
}