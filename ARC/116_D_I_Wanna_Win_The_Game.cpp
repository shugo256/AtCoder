...
#include <cassert>
#define MAX 10000  // 階乗をいくつまで計算するか
...
int main() {
    ...
    int dig;
    for (dig = 1; dig * 2 <= m; dig *= 2);

    std::vector dp(m+1, modlong(0));
    dp[m] = 1;
    for ( ; dig > 0; dig /= 2) {
        // std::cerr << dig << std::endl;
        for (int i=0; i<=m; i++) {
            int lim = std::min(n, i / dig);
            // std::cerr << i << ' ' << lim << std::endl;
            for (int j=2; j<=lim; j += 2) {
                dp[i - j * dig] += dp[i] * modComb(n, j);
            }
        }
        // for (auto &dpi:dp)
        //     std::cerr << dpi << ' ';
