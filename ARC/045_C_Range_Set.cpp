...
public:
    ...
    friend std::ostream &operator<<(std::ostream &os, const modlong &out) {
        os << out.val;
        return os;
    }
    friend std::istream &operator>>(std::istream &is, modlong &in) {
        ll inl;
        is >> inl;
...
int main() {
    int n, a, b ;
    std::cin >> n >> a >> b;
    std::vector dp(n+1, std::vector(std::max(a, b), std::vector(2, modlong())));
    dp[0][0][0] = 1;
    for (int i=0; i<n; i++) {
        for (int j=0; j<a; j++) {
            dp[i+1][std::max(j-1, 0)][0] += dp[i][j][0];
        }
        for (int j=0; j<b; j++) {
            dp[i+1][std::max(j-1, 0)][1] += dp[i][j][1];
        }
        for (int j=0; j<a && j<b; j++) {
            dp[i+1][j][0] += dp[i][0][1];
            dp[i+1][j][1] += dp[i][0][0];
        }
        for (int x=0; x<a; x++) std::cerr << dp[i+1][x][0] << ' ';
        std::cerr << std::endl;
        for (int x=0; x<b; x++) std::cerr << dp[i+1][x][1] << ' ';
        std::cerr << std::endl;
    }
    std::cout << dp[n][0][0] + dp[n][0][1] << '\n';
    return 0;
}