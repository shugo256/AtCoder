...
#include <cassert>
#define MAX 20000000  // 階乗をいくつまで計算するか
...
int main() {
    int n;
    cin >> n >> modlong::MOD;
    modlong ans = 0, rev3 = 1;
    for (int i=0; i<=n; i++) { // c, b, a の組の数(a < b < c)
        modlong rev2 = 1;
        for (int j=0; j+i<=n; j++) { // a, c, b or b, a, c の組の数(a < b < c)
            ans += rev3 * rev2;
            // cerr << rev3 << ' ' << rev2 << ' ' << i << ' ' << j << '\n';
            rev2 *= modComb(3*(n-i)-2*j, 2) / (j+1); 
        }
        if (i < n) rev3 *= modComb(3*(n-i), 3) / (i+1) * 2;
    }
    cout << ans << '\n';
    return 0;
}