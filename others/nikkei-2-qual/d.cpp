...
int main() {
    ...
    SegmentTree<long> sgt(n, "min");
    sgt.update(0, 0);
    for (auto &ei:es) {
        long mi = sgt.query(ei.l, ei.r);
        if (mi == numeric_limits<long>::max()) continue;
        //cerr << mi << ' ' << ei.l << ' ' << ei.r << ' ' << ei.c << '\n';
        if (mi + ei.c < sgt[ei.r])
            sgt.update(ei.r, mi + ei.c);
    }
    cout << (sgt[n-1] < numeric_limits<long>::max() ? sgt[n-1] : -1) << '\n';
    return 0;
}