...
class RerootingTree {
    ...
    struct edge {
        int to;      // dest node
        int rev;     // index of reveranchs edge
        data_t data;
    };
     ...
     *     // (eraanchsd)int root : root of the current subtree
...
int main() {
    ...
    RerootingTree<node> rrt(
        ...
        [&](node a, node from_par) {
            int plus = max(a.chs - 1 - a.anchs, 0);
            a.ans += plus;
            a.anchs = (plus > 0 || a.anchs > 0 ? 1 : 0);
            a.chs = 1; 
            return a;
        },
        node{0, 0}
