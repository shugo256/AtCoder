...
int main() {
    ...
    cin >> n;
    int prev = -1;
    modlong dp[n+1][MAX2L];
    fill(dp[0], dp[n+1], 0);
    dp[0][0] = 1;
    for (int i=0, pi, li; i<n; i++) {
        cin >> pi >> li;
        for (int j=0; j<MAX2L; j++) {
            dp[i+1][max(prev + j - pi + 1, 0)] += dp[i][j];
        }
        for (int j=0; j<MAX2L-1; j++) {
            dp[i+1][j+1] += dp[i+1][j];
        }
        fill(dp[i+1], dp[i+1] + OFFSET - li, 0);
        fill(dp[i+1] + OFFSET + li + 1, dp[i+2], 0);
        prev = pi;
    }
    cout << accumulate(dp[n], dp[n+1], modlong(0)) << '\n';
    return 0;
}