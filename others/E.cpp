...
int main() {
    ...
    std::vector dp(h+2, std::vector(w+2, std::vector<int>(4, -1)));
    for (int i=0; i<h; i++) {
        for (int j=0; j<w; j++) {
            if (s[i][j] == '.')
    ...
    pow2s[0] = 1;
    for (int i=1; i<=k; i++) {
        pow2s[i] = pow2s[i/2] * pow2s[i/2] * (i % 2 ? 2 : 1);
    }
    modlong ans = 0, all = modPow(2, k);
    for (int i=1; i<=h; i++) {
        for (int j=1; j<=w; j++) {
            if (s[i-1][j-1] == '#') continue;
            // std::cerr << dp[i][j][0] << ' ' << dp[i][j][1] << ' ' << dp[i][j][2] << ' ' << dp[i][j][3] << std::endl;
            int from = dp[i][j][0] + dp[i][j][1] + dp[i][j][2] + dp[i][j][3] + 1;
            ans += pow2s[k] - pow2s[k - from];
        }
    }
    std::cout << ans << '\n';
    return 0;
