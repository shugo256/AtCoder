...
int main() {
    ...
    for (int i=0; i<n; i++) {
        for (int j=0; j<=sum; j++) {
            if (j+a[i] <= sum) {
                dp[i+1][j+a[i]] += dp[i][j];
                half[i+1][j+a[i]] += half[i][j];
            }
            dp[i+1][j] += dp[i][j] + dp[i][j];
            half[i+1][j] += half[i][j];
        }
    }
    modlong sub = (sum % 2 ? 0 : dp[n][sum/2] - half[n][sum/2]);
    for (int i=sum/2+1; i<=sum; i++) sub += dp[n][i];
    cout << modlong(3).pow(n) - sub * 3 << '\n';
    return 0;
}