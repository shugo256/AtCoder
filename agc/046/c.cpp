...
int main() {
    ...
    for (auto &oi:one_cnt) cerr << oi << ' ';
    cerr << '\n';
    int zero_n = (int)one_cnt.size() - 1, one_n = (int)s.size() - zero_n;
    k = min(one_n, k);
    modlong dp[zero_n+2][one_n+1][k+1][2];
    fill(dp[0][0][0], dp[zero_n+2][0][0], 0);
    dp[0][0][0][0] = 1;
    for (int x=0; x<zero_n+1; x++) {
        for (int y=one_n; y>=0; y--) {
            for (int z=0; z<=k; z++) {
                for (int l=0; l<=one_cnt[x] && y + l <= one_n; l++) {
                    dp[x+1][y+l][z][0] += dp[x][y][z][0];
                }
                if (y > 0 && z < k)
                    dp[x][y-1][z+1][1] += dp[x][y][z][0] + dp[x][y][z][1];
                dp[x+1][y][z][0] += dp[x][y][z][1];
                // cerr << x << ' ' << y << ' ' << z << ' ' << dp[x][y][z][0] << endl;
            }
        }
    }
    modlong ans = 0;
    for (int z=0; z<=k; z++) {
        ans += dp[zero_n+1][0][z][0];
    }
    cout << ans << '\n';
    return 0;
}